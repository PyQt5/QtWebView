...
void QWebView2WebViewPrivate::initialize(HWND hWnd)
{
    ...
    const QString userDataFolder = QStandardPaths::writableLocation(QStandardPaths::AppLocalDataLocation) % QDir::separator() % QLatin1String("WebView2");
...
QString QWebView2WebViewPrivate::title() const
{
    if (m_webviewController) {
        ...
        Q_ASSERT_SUCCEEDED(hr);
        QString titleString = QString::fromStdWString(title);
        CoTaskMemFree(title);
        return titleString;
    }
    return QString();
                ...
                for (UINT i = 0; i < count; ++i) {
                    ...
                    if (SUCCEEDED(cookieList->GetValueAtIndex(i, &cookie))) {
                        ...
                        if (SUCCEEDED(cookie->get_Domain(&domainPtr)) &&
                            ...
                            QString domain = QString::fromStdWString(domainPtr);
                            QString name = QString::fromStdWString(namePtr);
                    ...
                    if (SUCCEEDED(cookieList->GetValueAtIndex(i, &cookie))) {
                        ...
                        if (SUCCEEDED(cookie->get_Domain(&domain))) {
                            if (SUCCEEDED(cookie->get_Name(&name))) {
                                emit cookieRemoved(QString::fromStdWString(domain), QString::fromStdWString(name));
                                CoTaskMemFree(name);
                            }
